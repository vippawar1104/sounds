...
async def main():
    ...
    llm_model        = os.getenv("AGENT_LLM_MODEL", "claude-haiku-4-5")
    ...
    async def on_data_received(transport_instance, data_packet, participant_id):
        ...
        try:
            ...
            if message.get("type") == "user_text":
                ...
                if text:
                    ...
                    # Publish user's text to transcript (for consistency)
                    await transcript_publisher._publish("user", text)
                    
                    # Add to context
                    context.add_message({"role": "user", "content": text})
                    
                    # Trigger LLM to generate response
                    logger.info(f"🤖 Triggering LLM to respond...")
                    await task.queue_frame(LLMContextFrame(context=context))
                    logger.info(f"✅ LLM triggered successfully")
